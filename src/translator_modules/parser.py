...
class Parser:
    ...
    def parse_statement(self):
        ...
        elif token[0] in {'INT', 'STRING_TYPE'}:  # Обработка объявлений int и string
    ...
    #         self.eat('ASSIGN')
    #         expr = self.parse_expression()
    #         self.eat('SEMI')  # Ожидается ';' после присваивания
    #         return VariableAssignNode(var_name, expr)
    #     else:
    #         self.error("Ожидался оператор присваивания '='")

    # def parse_declaration(self):
    #     """Парсинг объявления переменной"""
    #     self.eat('INT')  # Ожидаем ключевое слово 'int'
    #     var_name = self.current_token()[1]
    #     self.eat('ID')  # Ожидаем идентификатор переменной
    #     if self.current_token()[0] == 'ASSIGN':
    #         self.eat('ASSIGN')
    #         expr = self.parse_expression()
    #     else:
    #         expr = None  # Если значение не присвоено
    #     self.eat('SEMI')
    #     return VariableAssignNode(var_name, expr)
    ...
    def parse_expression(self):
        ...
        while self.current_token()[0] in {'LT', 'HT', 'LE', 'HE', 'PLUS', 'MINUS', 'MUL', 'DIV', 'EQ', 'NEQ', 'MOD',
                                          'ASSIGN'}:
    ...
    def parse_term(self):
        ...
        if token[0] == 'NUMBER':
        ...
        elif token[0] == 'STRING':  # Добавьте поддержку строк
            self.eat('STRING')
            ...
            self.eat('RPAREN')
            return expr
        elif token[0] == 'INPUT':  # Поддержка вызова функции input()
            self.eat('INPUT')
            self.eat('LPAREN')
            self.eat('RPAREN')
            return FunctionCallNode('input')
        else:
            self.error(f"Неожиданный токен в терме: {token}")
    ...
    def eat(self, token_type):
        ...
        token = self.current_token()
        # print(f"Ожидается: {token_type}, Найдено: {token[0]}")  # Вывод для отладки
