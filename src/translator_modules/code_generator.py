...
class CodeGenerator:
    ...
    def generate(self, node):
        if isinstance(node, ProgramNode):
            ...
            self.code.append({"opcode": "NOP", "comment": f"Not implemented for node {type(node).__name__}"})
    ...
    def generate_conditional_jump(self, operator, true_label, false_label):
        if operator == 'EQ':
            ...
            self.code.append({"opcode": Opcode.JA, "operand": true_label})
        else:
    ...
    def generate_numeric_comparison(self, node, while_loop_label, end_while_label):
        ...
        elif node.condition.operator == 'HT':
            self.code.append({"opcode": Opcode.JA, "operand": while_loop_label})
    ...
    def get_operand_info(self, operand):
        if isinstance(operand, IdentifierNode):
            ...
            return {"value": var_info["label"], "addr_mode": AddressingMode.indirect_addressing}
        elif isinstance(operand, NumberNode):
            return {"value": operand.value, "addr_mode": AddressingMode.immediate_addressing}
            ...
            return {"value": string_address, "addr_mode": AddressingMode.indirect_addressing, "type": "string"}
        else:
            raise Exception(f"Неизвестный тип операнда {type(operand)}")
    ...
    def generate_comparison_op(self, node, true_label):
        ...
        if node.operator == 'EQ':
            ...
            self.code.append({"opcode": Opcode.JB, "operand": true_label})
        elif node.operator == 'HT':
            self.code.append({"opcode": Opcode.JA, "operand": true_label})
        elif node.operator == 'LE':
            self.code.append({"opcode": Opcode.JBE, "operand": true_label})
        elif node.operator == 'GE':
            self.code.append({"opcode": Opcode.JAE, "operand": true_label})
        else:
    ...
    def load_input_and_compare(self, end_label, cmp_char, loop_label):
        ...
        self.code.append({"opcode": Opcode.CMP, "operand": cmp_char, "addr_mode": AddressingMode.immediate_addressing})
    ...
    def load_char_and_compare(self, end_label, loop_label):
        ...
        self.code.append({"opcode": Opcode.CMP, "operand": '\0', "addr_mode": AddressingMode.immediate_addressing})
    ...
    def generate_int_input(self):
        ...
        self.label_block.extend([
            {"label": start_input_label},
            {"label": end_input_label}
        ])
    ...
    def generate_variable_assign(self, node):
        ...
        if var_type == "unknown" and var_info:
            ...
            logging.info(f"Тип переменной {node.var_name} был найден: {var_type}")

        if isinstance(node.expression, BinaryOpNode):
            # Создаём или переиспользуем метку для переменной перед выполнением бинарной операции
            if not var_info:
                self.label_block.append({
                ...
                logging.info(f"Создание новой переменной: {node.var_name}, адрес: {len(self.data) - 1}")
            else:
                # Перезаписываем существующую переменную
                data_address = var_info["address"]
                logging.info(f"Переприсваивание переменной: {node.var_name}, адрес: {data_address}")
            ...
            if node.expression.func_name == "input":
                if var_type == "STRING_TYPE" or var_type == "string":
                    ...
                    if not var_info:
                        str_label = f"INP_STR_ADR_{self.string_label_counter}"
                        self.string_label_counter += 1
                        data_address = len(self.data)
                        ...
                        logging.info(f"Создание новой строковой переменной: {node.var_name}, адрес: {data_address}")
                    else:
                        data_address = var_info["address"]  # Переиспользуем существующий адрес
                        logging.info(f"Переприсваивание строковой переменной: {node.var_name}, адрес: {data_address}")

                        # Затираем оставшиеся символы предыдущей строки
                        old_string_length = len(self.data[data_address]["value"])
                        new_string_length = 15  # Максимальная длина строки
                        if old_string_length > new_string_length:
                            # Очищаем старые данные, обрезая строку
                            self.data[data_address]["value"] = [""] * new_string_length + ['\0']
                            logging.info(f"Оставшиеся символы строки затёрты для переменной: {node.var_name}")

                    # Генерация кода для input
                    self.generate_input(node.var_name)

                elif var_type == "INT":
                    # Обработка ввода для целочисленной переменной
                    ...
                    if not var_info:
                        ...
                        logging.info(f"Создание новой целочисленной переменной: {node.var_name}, адрес: {data_address}")
                    else:
                        data_address = var_info["address"]
                        logging.info(
                            f"Переприсваивание целочисленной переменной: {node.var_name}, адрес: {data_address}")
            ...
            if not var_info:
                ...
                data_address = var_info["address"]
                self.data[data_address] = {"type": "int", "value": node.expression.value}  # Обновляем значение
                logging.info(
                    f"Переприсваивание целочисленной переменной: {node.var_name}, новое значение: {node.expression.value}")
            ...
            if not var_info:
                ...
                logging.info(f"Создание новой строковой переменной: {node.var_name}, значение: {raw_string}")
            else:
                string_address = var_info["address"]
                old_string_length = len(self.data[string_address]["value"])
                new_string_length = len(raw_string)

                # Очищаем старые символы, если новые данные короче предыдущих
                if old_string_length > new_string_length:
                    self.data[string_address] = {"type": "string",
                                                 "value": [char for char in raw_string] + ['\0'] + [""] * (
                                                             old_string_length - new_string_length - 1)}
                    logging.info(f"Оставшиеся символы строки затёрты для переменной: {node.var_name}")
                else:
                    self.data[string_address] = {"type": "string", "value": [char for char in raw_string] + ['\0']}
                logging.info(f"Переприсваивание строковой переменной: {node.var_name}, новое значение: {raw_string}")

        else:
            # Генерация кода для других случаев (например, присваивание переменной)
            if var_info:
                # Если переменная существует, выполняем её обновление
                self.generate_binary_op(node.expression, node.var_name)
                logging.info(f"Обновление переменной: {node.var_name}")
            else:
                raise Exception(f"Переменная {node.var_name} не найдена для присваивания.")

    def save_to_file(self):
        # Сохраняем данные в JSON файл без комментариев
        code_output = {
        ...
        with open("output_code.json", "w", encoding="utf-8") as code_file:
