...
from src.isa import Opcode, AddressingMode
...
class Compiler:
    ...
    def _collect_data_labels(self, labels, data_section):
        ...
        print("Начало выполнения _collect_data_labels")
        print(f"labels: {labels}")
        print(f"data_section: {data_section}")
        print(f"Начальные значения: data_index={self.data_index}, current_address={self.current_address}")
    ...
    def _parse_instructions_file(self):
        with open(self.instructions_filename, 'r', encoding='utf-8') as file:
            data = json.load(file)
