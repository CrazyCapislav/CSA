...
class MemoryConfigurator:
    ...
    def load_from_json(self, json_file):
        ...
        if len(data_list) > self.data_memory_size:
            raise ValueError(
                f"Ошибка: количество данных ({len(data_list)}) превышает размер буфера для data ({self.data_memory_size})")
        ...
        if len(instructions) > self.instr_memory_size:
            raise ValueError(
                f"Ошибка: количество инструкций ({len(instructions)}) превышает размер буфера для instructions ({self.instr_memory_size})")
    ...
    def save_instructions_to_json(self, output_file):
        ...
        instructions_output = {
            'instructions': [
                ...
                for instr in self.instr_memory if instr is not None and isinstance(instr, dict) and 'opcode' in instr
            ]
        }
        with open(output_file, 'w') as file:
