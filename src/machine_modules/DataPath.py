...
from src.machine_modules.MUX import DAMux, AccMux, DRMux
...
class DataPath:
    ...
    def load_data_from_file(self, file_path):
        ...
        try:
            ...
            logging.error(f"Ошибка при декодировании JSON из файла {file_path}.")
    ...
    def load_data_from_json(self, json_data):
        ...
        for i, value in enumerate(data_list):
            ...
            if value == "CUR_ADR":
                ...
                # logging.info(f"Memory[{i}] initialized with {value}")
    ...
    def latch_acc(self, sel):
        ...
        # Проверяем, передана ли строка или объект AccMux
        if isinstance(sel, str):
            sel_mux = AccMux()  # Создаем объект AccMux
            sel_mux.handle_input(sel)  # Переключаем его состояние на строку
        else:
            sel_mux = sel  # Если это уже объект AccMux

        # Добавляем проверку на нейтральное состояние "NONE"
        if sel_mux.state == 'NONE':
            return  # Ничего не делаем, если состояние нейтральное
    ...
    def next_input(self):
        ...
        if self.cur_symbol < len(self.input_buffer):
            # Записываем текущий символ в память
            self.data_memory[self.input_adr] = self.input_buffer[self.cur_symbol]
            logging.info(
                f"Input[{self.cur_symbol}]: {self.input_buffer[self.cur_symbol]} added to memory at address {self.input_adr}")
            # Увеличиваем индекс для следующего символа
            self.cur_symbol += 1
        else:
            # Если достигнут конец input_buffer
    ...
    def alu_add(self):
        ...
        self.acc = self.alu
        self.update_flags(self.alu)  # Обновляем флаги на основе результата сложения
    ...
    def alu_div(self):
        ...
        if self.data_register == 0:
            raise ZeroDivisionError(f"Деление на ноль: data_register={self.data_register}")
        self.alu = self.acc // self.data_register  # Целочисленное деление
        self.update_flags(self.alu)
    ...
    def alu_compare(self):
        ...
        logging.info(f"Compare {self.acc} with {self.data_register}")
